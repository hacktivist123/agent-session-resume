...
Use `session_index.jsonl` to shortlist candidate session IDs by thread name, session name, and update time. Open full transcripts only after you have a short list of plausible session IDs, then confirm cwd, workdir, repo path, or mentioned files from the transcript itself. If the index is missing or inconclusive, then broaden discovery:
...
When choosing a session, prefer the most recent indexed session whose cwd, workdir, repo path, thread name, or mentioned files match the current repository. Do not pick the newest global Codex session if it appears to belong to a different project.
